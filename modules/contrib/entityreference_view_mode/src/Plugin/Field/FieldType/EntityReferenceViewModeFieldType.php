...
class EntityReferenceViewModeFieldType extends FieldItemBase {
  ...
  public static function availableBundles($type) {
    $bundles_raw = \Drupal::service('entity_type.bundle.info')->getBundleInfo($type);
    $bundles = [];
  ...
  public static function availableEntityTypes() {
    $definitions = \Drupal::service('entity_type.repository')->getEntityTypeLabels(TRUE);
  ...
  public static function availableViewModes($type) {
    ...
    $view_mode_classes = \Drupal::entityTypeManager()->getStorage('entity_view_mode')->loadMultiple($view_mode_ids);
