...
namespace Drupal\caching_module\Plugin\Block;
 ...
 * @Block(
 *  id = "related_items",
 *  admin_label = @Translation("Caching Related items"),
 * )
 */
class RelatedItems extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if ($current_node = $this->routeMatch->getParameter('node')) {
      ...
      $nids = $query->condition('field_name.value', $current_node->field_name->value)
        ->sort('created', 'DESC')
        ->range(0, 4)
        ->execute();
      ...
      $build = [
        '#theme' => 'caching_module_items_block',
        '#items' => $related_items,
      ];
